...
impl SdlEguiPainter {
    pub fn prewarm(&mut self, canvas: &mut sdl2::render::Canvas<sdl2::video::Window>) {
        for _ in 0..INITIAL_ICON_POOL_SIZE {
            if let Ok(mut texture) = canvas.create_texture_streaming(
    ...
    fn try_upload_icon(
        ...
        if created.is_err() && self.release_pools() > 0 {
            created = canvas.create_texture_streaming(
                PixelFormatEnum::RGBA32,
                MAX_ICON_SIDE,
                MAX_ICON_SIDE,
            );
    ...
    fn defer_image(
        ...
        scratch.clear();
        Self::fill_sdl_rgba(image, scratch);
        self.enqueue_pending(
            texture_id,
            PendingUpload {
                size: image.size(),
    ...
    fn upload(
        ...
        if pos.is_none() || !self.textures.contains_key(&texture_id) {
            ...
            if texture.is_err() && self.release_pools() > 0 {
                texture = canvas.create_texture_streaming(
                    PixelFormatEnum::RGBA32,
                    width as u32,
                    height as u32,
                );
...
mod tests {
    ...
    fn texture_coordinates_stay_inside_the_range_sdl_accepts() {
        let (x, y) =
            SdlEguiPainter::uv_for_test(egui::vec2(-1e-7, 1.0000002), egui::vec2(1.0, 1.0));
        assert_eq!((x, y), (0.0, 1.0));
        assert!(
            x.is_sign_positive(),
            "a negative zero is still out of bounds for SDL"
        );
