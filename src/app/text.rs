//! Catalog text arrives from three upstream databases and carries things the
//! bundled fonts cannot draw: emoji, variation selectors, CJK titles, HTML
//! entities. egui renders any of those as an empty box, so they are stripped
//! once at ingest rather than fought with at draw time.

/// Characters the bundled fonts can actually render. Deliberately narrow:
/// ASCII, Latin-1 Supplement, Latin Extended-A and a short list of typographic
/// punctuation that shows up constantly in homebrew descriptions.
fn is_renderable(c: char) -> bool {
    match c {
        '\n' => true,
        ' '..='~' => true,
        // Latin-1 Supplement minus the two invisible ones handled in `push_char`.
        '\u{00a1}'..='\u{00ff}' => true,
        // Latin Extended-A — Polish, Czech, Turkish authors.
        '\u{0100}'..='\u{017f}' => true,
        '\u{2018}' | '\u{2019}' | '\u{201c}' | '\u{201d}' => true, // curly quotes
        '\u{2013}' | '\u{2014}' => true,                           // en/em dash
        '\u{2026}' => true,                                        // ellipsis
        '\u{2022}' => true,                                        // bullet
        _ => false,
    }
}

/// True when `text` would survive [`sanitize`] untouched. Lets the common case
/// (plain ASCII, which is most of the catalog) skip all the work.
...
/// Expands the handful of HTML entities the upstream scrapers leave behind.
/// Returns the replacement and how many bytes of `rest` it consumed.
...
/// Strips everything the fonts can't draw and normalises whitespace, in place.
/// Idempotent, and a no-op allocation-wise when the text is already clean —
/// which matters because it runs over the whole catalog on every cold start.
...
pub fn sanitized(text: &str) -> String {
    ...
    while let Some(c) = rest.chars().next() {
        ...
        let replacement = match c {
            ...
            '\t' | '\r' | '\u{00a0}' => Some(' '),
            // Soft hyphen, zero-width space/joiners, BOM, variation selectors
            // (the `U+FE0F` in "⚠️" is what drew the stray box) and skin-tone
            // modifiers: all invisible by intent, all boxes in practice.
        ...
        // Collapse the runs of spaces that dropping a character tends to leave.
    ...
    // `trim_end` in place, keeping the single allocation we already made.
    let trimmed = out.trim_end().len();
    out.truncate(trimmed);
    out
}

/// A name that is safe to show. Japanese and Korean NPS titles sanitize down to
/// nothing, so fall back to the romanised name and finally the title ID rather
