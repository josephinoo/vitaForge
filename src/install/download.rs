...
/// Destination for downloaded bytes. Implemented by a plain file for regular
/// downloads and by the PKG extractor for in-app installs — either way,
/// `resume_offset` is what lets a retry or an app restart pick up where a
/// transfer left off instead of restarting from byte 0.
pub trait ByteSink: Send {
    fn resume_offset(&self) -> u64;
    fn write(&mut self, chunk: &[u8]) -> Result<()>;
    /// Called when the server ignored our `Range` header and sent the whole
    /// body again from byte 0 — discard whatever was written so far.
    fn reset(&mut self) -> Result<()>;
}

/// Plain-file sink used by the ordinary (non-PKG) download paths. Never
/// truncates on open — that was the original bug that made every retry of a
/// multi-GB download restart from zero.
pub struct FileSink {
    file: std::fs::File,
    written: u64,
}
...
pub enum ResumePlan {
    /// Keep going from `from`; `total` is the full content length if known.
    Continue { from: u64, total: Option<u64> },
    /// The server ignored `Range` (or none was sent) — start over.
    Restart { total: Option<u64> },
    /// `existing` already covers the whole file.
    AlreadyDone,
}

/// Pure decision function: given what's already on disk and how the server
/// responded, decide how to proceed. No network involved, so this is fully
/// unit-testable.
pub fn plan_resume(
    existing: u64,
    ...
    match status {
        StatusCode::PARTIAL_CONTENT => {
            // `bytes {start}-{end}/{total}` — trust the total from here over
            // `Content-Length`, which on a 206 is just the remaining size.
        ...
        StatusCode::RANGE_NOT_SATISFIABLE => {
            // Either we already have everything, or our on-disk copy is
            // stale/corrupt relative to what the server has now.
            let total = content_range
                .and_then(|range| range.rsplit('/').next())
                .and_then(|total| total.parse::<u64>().ok());
...
/// Sends progress at a bounded rate rather than once per HTTP chunk — the
/// difference between ~4 updates/second and hundreds of thousands over a
/// multi-GB transfer, each of which wakes the render loop.
...
impl<'a> ProgressReporter<'a> {
    ...
    /// Phase changes (resolving/extracting/etc.) always send immediately.
    pub fn phase(&mut self, progress: crate::install::Progress) {
...
/// Streams `url` into `sink`, resuming from `sink.resume_offset()` on every
/// attempt so a failure partway through never costs the bytes already
/// written. `rep.record` is called with the sink's absolute position after
/// every chunk (throttled internally).
